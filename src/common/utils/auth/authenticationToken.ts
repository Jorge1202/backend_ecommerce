...
import { AuthPayload, JwtOptions } from '../../interfaces/auth';
 ...
 * @returns Token JWT
 */
export const generateToken = ({ dataToken, expiresIn = '1d', secretType = 'access' }: JwtOptions): {Token:string, ExpiresIn:Date } => {
  /**
 ...
 */
  ...
  const payload: AuthPayload = dataToken;
...
export const verifyToken = async (token: string, secretType: secretType = 'access'):
  Promise<{ error: boolean; message: string; status: number; payload?: AuthPayload }> => {
  try {
    if (!token) {
...
const getToken = async (token: string, secretType: string): Promise<{ error: boolean; message: string; status: number; payload: AuthPayload | null }> => {
  try {
 ...
 */
async function decodeHeader(authorization: string, secretType: string): Promise<{ error: boolean; status: number; message: string; payload: AuthPayload | null }> {
  try {
    // 🔹 Extraer el token
 ...
 */
async function verifyJWT(token: string, secretType: string): Promise<{ error: boolean; status: number; message: string; payload?: AuthPayload }> {
  try {
